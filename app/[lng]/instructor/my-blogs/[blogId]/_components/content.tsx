...
import { blogContentSchema } from "@/lib/validation";
      ...
      <CardContent className="relative p-6">
        ...
        {state ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="line-clamp-6 whitespace-pre-line font-medium">
              {blog.content}
            </span>
          </div>
        )}
...
function Forms({ blog, onToggle }: FormsProps) {
  ...
  const onSubmit = (values: z.infer<typeof blogContentSchema>) => {
    setIsLoading(true);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Textarea {...field} className="h-64" disabled={isLoading} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
